...
int tickets_left = 50;
int socket_fd, movie_fd;
struct sockaddr_in movie_server;
sem_t lock;
struct hostent* host;
...
int main(int argc, char ** argv){
  ...
  struct sockaddr_in play_server, client_addr;
  ...
  play_server.sin_addr.s_addr = htonl(INADDR_ANY);
  ...
  if((return_val = bind(socket_fd, (struct sockaddr *) &play_server, sizeof(play_server))) < 0){
...
void *kiosk_handler(void *ptr){
  ...
  while(1){
    ...
    switch(msg_type) {
    case 'b' :
      sscanf(msg->buf, "%c %d", &msg_type, &kiosk_id);
      break;
    case 'p' :
	...
	exit(EXIT_FAILURE);
      ...
      if((return_val = send(movie_fd, msg->buf, CAPACITY, 0)) < 0){
	perror("Send failed");
	exit(EXIT_FAILURE);
      }
      if((return_val = recv(movie_fd, msg->buf, CAPACITY, 0)) ==  0){
